use crate::{
    configs::{ClockConfig, OmniPaxosKVConfig},
    database::Database,
    network::Network,
};
use chrono::Utc;
...
fn clock_from_config(server_id: NodeId, clock_cfg: Option<&ClockConfig>) -> Clock<Command> {
    let (uncertainty_us, sync_ms) = clock_cfg
        .map(|c| (c.clock_uncertainty_us, c.clock_sync_interval_ms))
        .unwrap_or((10, 1));

    // Model uncertainty as per-tick drift bound in microseconds; sync_freq is in ms.
...
impl OmniPaxosServer {
    pub async fn new(config: OmniPaxosKVConfig) -> Self {
        ...
        let clock = clock_from_config(config.local.server_id, clock_cfg.as_ref());
